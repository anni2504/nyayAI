...
import { UserCheck, Video, Calendar, ArrowUpRight, ShieldCheck } from 'lucide-react';
  ...
  const authorizedClients = [
    {
      ...
      bookingId: 'bk-501',
      name: 'Rohan Sharma',
      matter: 'Neighbour Boundary Dispute & Emergency Injunction Order',
      status: 'Active Consultation Ready',
      lastInteraction: 'Today, 2:15 PM',
      jurisdiction: 'Bengaluru, Karnataka',
      consultationDate: 'Today (4:30 PM - 5:30 PM)',
      canJoin: true
    },
    {
      id: 'cli-2',
    ...
    <div className="flex-1 bg-[#080D1F] text-white p-4 sm:p-6 lg:p-8 overflow-y-auto space-y-6 select-none font-sans">
      ...
      <div className="flex items-center justify-between border-b border-[#29215F]/80 pb-4">
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-white mt-1">My Client Directory & Consultations</h1>
        </div>
      ...
      <div className="space-y-4">
        {authorizedClients.map((client) => (
          <div key={client.id} className="bg-gradient-to-br from-[#0D132D] via-[#080D1F] to-[#060913] p-6 rounded-2xl border border-[#29215F]/80 border-t-[#5146D8]/30 space-y-4 shadow-xl">
            ...
            <div className="p-3.5 bg-[#060913] rounded-xl border border-[#29215F]/60 text-xs space-y-1.5">
              <strong className="text-white block font-extrabold">{client.matter}</strong>
              <div className="flex items-center space-x-4 text-slate-400 text-[11px]">
                <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5 text-[#F4B400]" /> {client.consultationDate}</span>
                <span>•</span>
            ...
            <div className="flex items-center justify-between pt-1">
              <div className="flex items-center space-x-2 text-[11px] font-mono text-emerald-400">
                ...
                <span>Verified Representation Agreement</span>
              </div>

              <div className="flex items-center space-x-3">
                {client.canJoin && (
                  <button
                    onClick={() => handleJoinCall(client.bookingId)}

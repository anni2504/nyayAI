  ...
  return (
    <div className="flex-1 bg-slate-950 text-white p-4 sm:p-6 lg:p-8 overflow-y-auto space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <div>
          <span className="text-[10px] font-bold uppercase tracking-widest text-amber-400">Client Intake</span>
          <h1 className="text-2xl font-extrabold text-white">Client Requests</h1>
        </div>
      ...
      <div className="p-4 bg-amber-400/10 border border-amber-400/20 rounded-2xl flex items-start space-x-3 text-xs text-amber-300">
        ...
        <div>
          <strong className="block font-bold text-amber-200">Client Privacy & Matter Protection:</strong>
      ...
      {leads.length === 0 ? (
        <div className="bg-slate-900 rounded-3xl border border-slate-800 p-12 text-center space-y-3 max-w-md mx-auto my-12">
          ...
          <h3 className="text-base font-extrabold text-white">No client requests yet.</h3>
          ...
          {leads.map((lead) => (
            <div key={lead.id} className="bg-slate-900 p-6 rounded-2xl border border-slate-800 space-y-4">
              ...
              <div className="p-3 bg-slate-950 rounded-xl border border-slate-800/80 text-xs space-y-1">
                ...
                {lead.matchReasons?.map((reason: string, idx: number) => (
                ...
                <div className="flex items-center space-x-3">
                  {lead.status === 'pending' ? (
                    ...
                    <span className="text-xs font-bold text-emerald-400 bg-emerald-950 px-3 py-1 rounded border border-emerald-800">
                      Request {lead.status === 'accepted' ? 'Accepted' : 'Declined'}
                    </span>
                  )}
                </div>
              </div>

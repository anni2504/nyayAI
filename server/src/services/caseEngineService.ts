...
export function classifyMessageIntent(text: string, caseState: CaseState): MessageIntent {
  ...
  // 1. READINESS MANIPULATION ATTEMPT
  if (
  ...
  // 3. GREETING
  const greetingPhrases = ['hi', 'hello', 'hey', 'good morning', 'good afternoon', 'good evening', 'start', 'help'];
  if (greetingPhrases.includes(stripped) || stripped.length <= 2) {
    return 'GREETING';
  }

  // 4. CASUAL CONVERSATION / OUT OF SCOPE
  if (
  ...
  // 6. CASE INTAKE / CASE FACT UPDATE
...
export function mergeFactsDeterministically(existing: CaseFacts, newText: string, lastAssistantMsg?: string, isDoc = false): CaseFacts {
  ...
  if ((text === 'yes' || text === 'yes.') && lastAssistantMsg) {
  ...
  // 2. Matter Clarity
  if (text.includes('fight') || text.includes('assault') || text.includes('neighbour') || text.includes('dispute') || text.includes('boundary') || text.includes('altercation') || text.includes('road') || text.includes('hit me')) {
  ...
  } else if (text.includes('builder') || text.includes('flat') || text.includes('possession') || text.includes('rera') || text.includes('deliver')) {
    const isDetailed = text.length > 30;
    merged.matter = {
  ...
  if (text.includes('road') || text.includes('hit me') || text.includes('no reason') || text.includes('walk') || text.includes('struck')) {
    merged.incidentDescription = {
      value: 'Client states neighbour struck them while on the road without provocation.',
      source,
      confidence: 0.9,
      completeness: 0.75,
  ...
  // 4. Timeline & Dates
  if (text.includes('2 years') || text.includes('22 months') || text.includes('2024') || text.includes('2023') || text.includes('yesterday') || text.includes('last week') || text.includes('today')) {
    merged.timeline = {
  ...
  // 5. Jurisdiction (Preserve & Merge variations without overwriting)
  ...
  // 6. Police Status
  if (text.includes('reported it to police') || text.includes('reported to police') || text.includes('csr filed') || text.includes('csr') || text.includes('fir')) {
    merged.policeStatus = {
      value: true,
      source,
      confidence: 0.95,
  ...
  if (text.includes('injury') || text.includes('injuries') || text.includes('injured') || text.includes('assault') || text.includes('physical violence') || text.includes('threat') || text.includes('damage') || text.includes('hit')) {
...
export async function processClientTurn(
  ...
  // INTENT 1: READINESS MANIPULATION ATTEMPT
  ...
  // INTENT 3: CASUAL CONVERSATION / OUT OF SCOPE
  ...
  // INTENT 5: CASE_INTAKE or CASE_FACT_UPDATE -> Merge Facts Deterministically
...
NEVER ask for a known fact again. Ask client about UNKNOWN parameters: ${state.missingInformation.join(', ')}.`
  ...
  try {
    ...
    if (state.facts.matter.value === 'Neighbour Dispute / Physical Altercation') {
      ...
      } else if (userMessage.toLowerCase().includes('road') || userMessage.toLowerCase().includes('hit me') || userMessage.toLowerCase().includes('no reason')) {
